...
function classCard(s){
  var mode = (s.mode || 'offline').toLowerCase();
  if (mode !== 'online' && mode !== 'offline' && mode !== 'hybrid') mode = 'offline';
  var batchName = s.batch ? (s.batch.name || '') : '';
  var instructor = s.instructor ? ((s.instructor.firstName||'')+' '+(s.instructor.lastName||'')).trim() : '';
  var modeLabel = mode === 'online' ? 'Online' : mode === 'hybrid' ? 'Hybrid' : 'Offline';
  var action;
  if (mode === 'offline') {
    action = '<div class="class-join join-offline">&#128205; '+(s.venue || 'Campus — Hyderabad')+'</div>';
  } else {
    var link = s.meetingLink || '#';
    action = '<a class="class-join join-online" href="'+link+'" target="_blank" onclick="if(this.getAttribute(\'href\')===\'#\'){event.preventDefault();toast(\'Meeting link will be shared before class\',\'info\');}">&#9654; Join '+modeLabel+' Class</a>';
  }
  return '<div class="class-card '+(mode==='hybrid'?'online':mode)+'">'+
    '<div class="class-top">'+
      '<div class="class-title">'+(s.title || 'Class')+'</div>'+
      '<span class="mode-badge mode-'+mode+'"><span class="dot"></span>'+modeLabel+'</span>'+
    '</div>'+
    '<div class="class-meta">'+
      '<span>&#128197; <b>'+fmtDate(s.startTime)+'</b></span>'+
      '<span>&#128336; <b>'+fmtTime(s.startTime)+' – '+fmtTime(s.endTime)+'</b></span>'+
    '</div>'+
    (s.agenda && s.agenda.length ? '<div class="sched-covers" style="margin-top:.1rem"><b>Covers:</b> '+s.agenda.slice(0,3).join(' &bull; ')+'</div>' : '')+
    (batchName||instructor ? '<div class="class-meta">'+[batchName?'&#128218; '+batchName:'',instructor?'&#128100; '+instructor:''].filter(Boolean).join('<span></span>')+'</div>' : '')+
    action+
  '</div>';
}
...
function liveClassCard(s){
  var isLive = s.status === 'live';
  var join = s.meetingLink || '#';
  var badge = isLive
    ? '<span class="mode-badge mode-live"><span class="dot"></span>Live Now</span>'
    : '<span class="mode-badge mode-online"><span class="dot"></span>Upcoming</span>';
  return '<div class="class-card '+(isLive?'live':'online')+'">'+
    '<div class="class-top"><div class="class-title">'+s.title+'</div>'+badge+'</div>'+
    '<div class="class-meta"><span>&#128197; <b>'+fmtDate(s.startTime)+'</b></span><span>&#128336; <b>'+fmtTime(s.startTime)+' &ndash; '+fmtTime(s.endTime)+'</b></span></div>'+
    '<a class="class-join '+(isLive?'join-live':'join-online')+'" href="'+join+'" target="_blank" onclick="if(this.getAttribute(\'href\')===\'#\'){event.preventDefault();toast(\'Link shared 15 min before class\',\'info\');}">'+
      (isLive ? '&#128308; Join Live Now' : '&#9654; Join Class')+'</a>'+
  '</div>';
}
function recordingCard(s){
  var title = (s.title||'Recording').replace(/'/g,'&#39;');
  return '<div class="class-card offline">'+
    '<div class="rec-thumb" onclick="watchRecording(\''+(s.recordingUrl||'')+'\',\''+title+'\')">'+
      '<div class="play"><svg viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3"/></svg></div>'+
    '</div>'+
    '<div class="class-top"><div class="class-title">'+s.title+'</div><span class="mode-badge mode-recorded"><span class="dot"></span>Recorded</span></div>'+
    '<div class="class-meta"><span>&#128197; '+fmtDate(s.startTime)+'</span>'+(s.venue?'<span>&#128205; '+s.venue+'</span>':'')+'</div>'+
    '<button class="class-join join-recorded" onclick="watchRecording(\''+(s.recordingUrl||'')+'\',\''+title+'\')">&#9654; Watch Recording</button>'+
  '</div>';
}
function inPersonCard(s){
  return '<div class="class-card offline">'+
    '<div class="class-top"><div class="class-title">'+s.title+'</div><span class="mode-badge mode-offline"><span class="dot"></span>In-Person</span></div>'+
    '<div class="class-meta"><span>&#128197; <b>'+fmtDate(s.startTime)+'</b></span><span>&#128336; <b>'+fmtTime(s.startTime)+'</b></span></div>'+
    '<div class="class-join join-offline">&#128205; '+(s.venue||'Campus — Hyderabad')+'</div>'+
  '</div>';
}
...
function markAnnRead(id) {
  apiFetch('/announcements/' + id + '/read', { method: 'POST' }).catch(function(){});
}
...
function renderExams(){
  return `
  <div class="section-head"><span class="section-title">Upcoming Exams</span></div>
  <div class="ann-list">
    ${[['Aptitude Test','22 May 2024','Online','Quantitative & Logical Reasoning'],['Technical Quiz - React','28 May 2024','In-Person','React fundamentals and advanced hooks'],['Final Assessment - Full Stack','15 Jun 2024','In-Person','Comprehensive full stack evaluation']].map(([n,d,m,t])=>`
    <div class="ann-item">
      <div class="ann-dot" style="background:var(--amber)"></div>
      <div style="flex:1">
        <div style="display:flex;justify-content:space-between">
          <div class="ann-title">${n}</div>
          <span class="badge b-pending">${d}</span>
        </div>
        <div class="ann-body" style="margin-top:.2rem">${t} &bull; ${m}</div>
      </div>
    </div>`).join('')}
  </div>`;
}
...
function markComplete(btn){ btn.textContent='✓ Completed'; btn.disabled=true; btn.style.opacity='.7'; toast('Topic marked complete','success'); }
